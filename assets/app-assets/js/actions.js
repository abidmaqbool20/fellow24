...
$(document).on("click",".open-model",function(){
	if(internet_connection()){
    	...
    	$search_string = $url.search;
 

		$.post($base_url+'/app/save_request',{data:$data,csrf_token:$csrf_token,'request_type':'model'},function(request_id){
			if(request_id){
				console.log($search_string+"&"+request_id);
				$modal_url = $base_url+'/app'+$search_string+"&"+request_id;
		 		history.pushState(null, null, $modal_url);
				load_modal(request_id);
			}
			else{
				toast_error_message('Failed!',"It seems you don't have internet connection. Please check!");
			}
...
function load_modal($request_id){
	if(internet_connection()){
		if($request_id > 0){
			add_loader();
			$('body').append('<div class="model-'+$request_id+'"></div>');
		    $(".model-"+$request_id).html('').load($base_url+"/app/load_form",{request_id:$request_id,csrf_token:$csrf_token},function(){
		    	remove_loader();
...
$(document).on("click",".close-model",function(){ 
	$(this).closest(".model-first").remove(); 
	$url = new URL(window.location.href); 
    $request_ids = $url.search.slice(1).split('&');
    $request_ids.pop();
    
    if($request_ids.length > 1){   
    	$url_request_ids = $request_ids.join('&');
		$browser_url = $base_url+"/app?"+$url_request_ids;
		history.pushState(null, null, $browser_url);
		$last_request_id = $request_ids.pop();
		$(".model-"+$last_request_id).remove();
		load_modal($last_request_id);
	}
	else{
		$(".model-first").removeClass('show'); 
		$(".page-overlay").removeClass("show");
		$request_ids = $request_ids.join('&');
		$browser_url = $base_url+"/app?"+$request_ids;
		history.pushState(null, null, $browser_url);
...
$(window).on("load",function(){
    ...
    $request_ids = $url.search.slice(1).split('&');
   	...
   	if($request_ids.length >= 1){
   		load_page($request_ids[0],false); 
   		load_modal($last_request_id);
   	}
   	else{
   		$(".page-overlay").removeClass("show");
   		$(".model-first").removeClass('show'); 
   		load_page($last_request_id,false); 
   	} 
});
...
$(document).on("click",".table-column",function(){
	if($(this).is(':checked')){
		$(this).prop('checked',true);
		$('.page-table').find('.col-'+$(this).val()).removeClass('hide');
	}else{
		$(this).prop('checked',false);
		$('.page-table').find('.col-'+$(this).val()).addClass('hide');
	}
...
$(document).on("click","#table-sortable th",function(){
	$this = $(this);
	 $(this).parent('tr').find('th').each(function(i){
